...
const SWIPE_ZONE = 40; // The area on the right edge where swipe-to-open starts
...
export default function AppLayout({ children, title, breadcrumb }: AppLayoutProps) {
  ...
  // slideAnim starts at DRAWER_WIDTH (hidden off-screen to the right)
  ...
  const closeDrawer = () => {
    Animated.timing(slideAnim, {
      toValue: DRAWER_WIDTH, duration: 200, useNativeDriver: true,
  ...
  const panResponder = useRef(
    PanResponder.create({
      onMoveShouldSetPanResponder: (_, gestureState) => {
        ...
        if (!drawerOpen) {
          // Detect swipe from Right edge to Left (Open)
          return isHorizontal && gestureState.moveX > (SCREEN_WIDTH - SWIPE_ZONE) && gestureState.dx < -10;
        } else {
          // Detect swipe from Left to Right (Close)
          return isHorizontal && gestureState.dx > 10;
        }
      },
      onPanResponderMove: (_, gestureState) => {
        if (!drawerOpen) {
          // Optional: Track finger movement for open
          const newVal = DRAWER_WIDTH + gestureState.dx;
          slideAnim.setValue(Math.max(0, newVal));
        } else {
          // Track finger movement for close
          const newVal = Math.max(0, gestureState.dx);
  ...
  const openNotif = () => {
    ...
    Animated.spring(notifAnim, {
      toValue: 1, useNativeDriver: true, tension: 110, friction: 13,
    }).start();
  ...
  const closeNotif = () => {
    Animated.timing(notifAnim, {
      toValue: 0, duration: 170, useNativeDriver: true,
  ...
  return (
    <View style={S.root} {...panResponder.panHandlers}>
      ...
      {/* ── Top Header (Logo Left, Notif & Hamburger Right) ── */}
      <View style={S.header}>
              ...
              <View style={S.breadcrumb}>
                {breadcrumb.map((crumb, i) => (
                  <View key={i} style={S.breadcrumbItem}>
                    ...
                    <Text style={[S.breadcrumbText, i === breadcrumb.length - 1 && S.breadcrumbActive]}>
                      {crumb}
                    </Text>
                  </View>
                ))}
      ...
      {notifOpen && (
        <Animated.View style={[S.notifPanel, { opacity: notifAnim, transform: [{ translateY: notifAnim.interpolate({inputRange:[0,1], outputRange:[-10, 0]}) }] }]}>
          ...
          return (
            ...
            >
              <Ionicons
                name={item.icon}
                size={18}
                color={active ? '#fff' : '#99aabb'}
              />
              <Text style={[S.drawerItemText, active && S.drawerItemTextActive]}>
                {item.label}
              </Text>
            </TouchableOpacity>
          );
...
const S = StyleSheet.create({
  ...
  header: {
    ...
    borderBottomWidth: 1, borderBottomColor: '#e8e8e8', zIndex: 10,
  },
  logoRow: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  logoIconWrap: {
    width: 30, height: 30, backgroundColor: '#1a2e4a',
  ...
  // Drawer (Positioned Right)
  drawer: {
    position: 'absolute', top: 0, right: 0, bottom: 0,
    width: DRAWER_WIDTH, backgroundColor: '#2c3e50',
    zIndex: 100, paddingTop: STATUS_BAR_HEIGHT,
    shadowColor: '#000', shadowOffset: { width: -3, height: 0 },
    shadowOpacity: 0.1, shadowRadius: 5, elevation: 10,
  },
  drawerOverlay: {
    ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(0,0,0,0.4)', zIndex: 90,
  },
  drawerTop: {
    flexDirection: 'row', alignItems: 'center',
    paddingHorizontal: 20, paddingVertical: 25, gap: 12,
  },
  drawerAvatar: {
  ...
  drawerFooter: {
    ...
    borderTopWidth: 1, borderTopColor: '#3d5166',
  ...
  notifPanel: {
    position: 'absolute', top: STATUS_BAR_HEIGHT + 60, right: 16,
    width: 280, borderRadius: 15, overflow: 'hidden', zIndex: 50,
